...
Q_table_eps = Q_learning_eps.initialisation()
Q_table_ucb = Q_learning_ucb.initialisation()
...
while epoch < 10000:
    ...
    if maze.running:
        ...
        Q_table_eps = Q_learning_eps.update(Q_table=Q_table_eps
                                        , state=state
                                        , command=command
                                        , reward=reward)
        Q_table_ucb = Q_learning_ucb.update(Q_table=Q_table_ucb
                                            , state=state
                                            , command=command
                                            , reward=reward)
        state += 1
